...
public class IronGolemStaffProjectileEntity extends StaffProjectileEntity implements IStaffProjectile {
    public IronGolemStaffProjectileEntity(EntityType<? extends ProjectileItemEntity> p_i50155_1_, World p_i50155_2_) {
        super(p_i50155_1_, p_i50155_2_);
    }

    public IronGolemStaffProjectileEntity(double x, double y, double z, World world) {
        super(SummonerClassEntitiesRegister.IRON_GOLEM_STAFF_PROJECTILE.get(), x, y, z, world);
    }

    public IronGolemStaffProjectileEntity(LivingEntity entity, World world) {
        super(SummonerClassEntitiesRegister.IRON_GOLEM_STAFF_PROJECTILE.get(), entity, world);
    }

    @Override
    protected Item getDefaultItem() {
        return Items.IRON_INGOT.asItem();
    }
    ...
    protected void onHitEntity(EntityRayTraceResult traceResult) {
        if (!level.isClientSide){
            if (traceResult.getEntity() instanceof LivingEntity && this.getOwner() != null
                ...
                int golemNumber = 1 + bonusMinion;
                for (int num = 1; num <= golemNumber; num++){
                    int xAdjust = roll.nextInt(3) - 1;
                    int yAdjust = roll.nextInt(3);
                    int zAdjust = roll.nextInt(3) - 1;
                    SummonedIronGolemEntity golem = new SummonedIronGolemEntity(SummonerClassEntitiesRegister.SUMMONED_IRON_GOLEM.get(), level);
                    if (xAdjust == 0 && zAdjust == 0){
                        golem.setPos(this.getOwner().position().x,
                                this.getOwner().position().y + 3D,
                                this.getOwner().position().z);
                    }
                    else {
                        golem.setPos(this.getOwner().position().x + xAdjust,
                                this.getOwner().position().y + yAdjust,
                                this.getOwner().position().z + zAdjust);
                    }
                    golem.setAttackTarget(livingEntity, (LivingEntity) this.getOwner());
                    golem.setCustomName(new StringTextComponent(this.getOwner().getScoreboardName() + "'s iron golem"));
                    giveMinionEnchantment(golem);
    ...
    private boolean hasFullArmorSet(PlayerEntity player) {
        ...
        for (ItemStack stack : armorList){
            ...
            if (!(stack.getItem() instanceof IronGolemArmor)
