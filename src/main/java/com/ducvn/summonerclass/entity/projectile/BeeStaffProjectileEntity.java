...
public class BeeStaffProjectileEntity extends StaffProjectileEntity implements IStaffProjectile {
    public BeeStaffProjectileEntity(EntityType<? extends ProjectileItemEntity> p_i50155_1_, World p_i50155_2_) {
        super(p_i50155_1_, p_i50155_2_);
    }

    public BeeStaffProjectileEntity(double x, double y, double z, World world) {
        super(SummonerClassEntitiesRegister.BEE_STAFF_PROJECTILE.get(), x, y, z, world);
    }

    public BeeStaffProjectileEntity(LivingEntity entity, World world) {
        super(SummonerClassEntitiesRegister.BEE_STAFF_PROJECTILE.get(), entity, world);
    }

    @Override
    protected Item getDefaultItem() {
        return Items.HONEYCOMB.asItem();
    }
    ...
    protected void onHitEntity(EntityRayTraceResult traceResult) {
        if (!level.isClientSide){
            if (traceResult.getEntity() instanceof LivingEntity && this.getOwner() != null
                ...
                int beeNumber = roll.nextInt(10) + 1 + bonusMinion;
                ...
                for (int num = 1; num <= beeNumber; num++){
                    ...
                    SummonedBeeEntity bee = new SummonedBeeEntity(SummonerClassEntitiesRegister.SUMMONED_BEE.get(), level);
                    if (roll.nextInt(100) < 1){
                        bee.setEasterEgg();
                    }
                    if (xAdjust == 0 && zAdjust == 0){
                        bee.setPos(this.getOwner().position().x,
                                this.getOwner().position().y + 3D,
                                this.getOwner().position().z);
                    }
                    else {
                        bee.setPos(this.getOwner().position().x + xAdjust,
                                this.getOwner().position().y + yAdjust,
                                this.getOwner().position().z + zAdjust);
                    }
                    bee.setAttackTarget(livingEntity, (LivingEntity) this.getOwner());
                    bee.setCustomName(new StringTextComponent(this.getOwner().getScoreboardName() + "'s bee"));
                    giveMinionEnchantment(bee);
