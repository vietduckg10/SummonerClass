...
public class PhantomStaffProjectileEntity extends StaffProjectileEntity implements IStaffProjectile {
    public PhantomStaffProjectileEntity(EntityType<? extends ProjectileItemEntity> p_i50155_1_, World p_i50155_2_) {
        super(p_i50155_1_, p_i50155_2_);
    }

    public PhantomStaffProjectileEntity(double x, double y, double z, World world) {
        super(SummonerClassEntitiesRegister.PHANTOM_STAFF_PROJECTILE.get(), x, y, z, world);
    }

    public PhantomStaffProjectileEntity(LivingEntity entity, World world) {
        super(SummonerClassEntitiesRegister.PHANTOM_STAFF_PROJECTILE.get(), entity, world);
    }
    ...
    protected void onHitEntity(EntityRayTraceResult traceResult) {
        if (!level.isClientSide){
            if (traceResult.getEntity() instanceof LivingEntity && this.getOwner() != null
                ...
                int numPhantom = 2 + bonusMinion;
                for (int num = 1; num <= numPhantom; num++){
                    int xAdjust = roll.nextInt(3) - 1;
                    int yAdjust = roll.nextInt(3);
                    int zAdjust = roll.nextInt(3) - 1;
                    SummonedPhantomEntity phantom = new SummonedPhantomEntity(SummonerClassEntitiesRegister.SUMMONED_PHANTOM.get(), level);
                    if (xAdjust == 0 && zAdjust == 0){
                        phantom.setPos(this.getOwner().position().x,
                                this.getOwner().position().y + 3D,
                                this.getOwner().position().z);
                    }
                    else {
                        phantom.setPos(this.getOwner().position().x + xAdjust,
                                this.getOwner().position().y + yAdjust,
                                this.getOwner().position().z + zAdjust);
                    }
                    phantom.setAttackTarget(livingEntity, (LivingEntity) this.getOwner());
                    phantom.setCustomName(new StringTextComponent(this.getOwner().getScoreboardName() + "'s phantom"));
                    giveMinionEnchantment(phantom);
