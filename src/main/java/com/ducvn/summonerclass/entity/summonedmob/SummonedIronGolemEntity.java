...
public class SummonedIronGolemEntity extends IronGolemEntity implements ISummonedEntity {
    private static final DataParameter<Boolean> DATA_EASTER_EGG_ID = EntityDataManager.defineId(SummonedIronGolemEntity.class, DataSerializers.BOOLEAN);
    private UUID target;
    private UUID master;
    ...
    public SummonedIronGolemEntity(EntityType<? extends SummonedIronGolemEntity> p_i225714_1_, World p_i225714_2_) {
    ...
    public boolean isInvisibleTo(PlayerEntity player) {
        if (player.getScoreboardName().equals(this.getCustomName().getString().replace("'s iron golem",""))){
            return false;
        }
        else {
    ...
    protected void onEffectAdded(EffectInstance effectInstance) {
        if (hasFullArmorSet(level.getPlayerByUUID(master))
                && effectInstance.getEffect().getCategory() == EffectType.HARMFUL)
        {
    ...
    private boolean hasAdvanceArmorSet(PlayerEntity player){
        ...
        for (ItemStack stack : armorList){
            if (!(stack.getItem() instanceof AdvancedIronGolemArmor)){
                return false;
            }
        }
        return true;
        ...
        if (!level.isClientSide){
            ...
            || level.getPlayerByUUID(master) == null) {
                ...
                || !((ServerWorld) level).getEntity(target).isAlive()){
                    ...
                    if ((isMagnetize || hasAdvanceArmorSet(level.getPlayerByUUID(master))) && tickCount % 10 == 0){
