...
import com.ducvn.summonerclass.item.armor.advanced.AdvancedPolarBearArmor;
import com.ducvn.summonerclass.item.armor.basic.PolarBearArmor;
...
public class SummonedPolarBearEntity extends PolarBearEntity implements ISummonedEntity {
    private UUID target;
    private UUID master;
    ...
    public SummonedPolarBearEntity(EntityType<? extends SummonedPolarBearEntity> p_i225714_1_, World p_i225714_2_) {
    ...
    public boolean isInvisibleTo(PlayerEntity player) {
        if (player.getScoreboardName().equals(this.getCustomName().getString().replace("'s polar bear",""))){
            return false;
        }
        else {
    ...
    @Override
    public boolean hurt(DamageSource source, float damageTaken) {
        ...
        if (master != null){
            if (source.getEntity() instanceof LivingEntity && (hasThorn || hasFullArmorSet(level.getPlayerByUUID(master)))
            && !(source.getEntity() instanceof PlayerEntity && level.getPlayerByUUID(master).equals(source.getEntity()))){
                float reflectDamage = 0;
                if (hasFullArmorSet(level.getPlayerByUUID(master))){
                    reflectDamage = reflectDamage + damageTaken;
                }
                if (source.getEntity() instanceof LivingEntity && hasThorn){
                    reflectDamage = reflectDamage + SummonerClassConfig.minion_thorn_damage.get().floatValue();
                }
                source.getEntity().hurt(DamageSource.thorns(null), reflectDamage);
            }
            if (hasAdvanceArmorSet(level.getPlayerByUUID(master))){
                Random roll = new Random();
                if (roll.nextInt(10) < 3){
                    this.addEffect(new EffectInstance(Effects.REGENERATION, 600));
                }
            }
        }
        ...
        return super.hurt(source, damageTaken);
    }
    ...
    private boolean hasFullArmorSet(PlayerEntity player) {
        ...
        for (ItemStack stack : armorList){
            ...
            if (!(stack.getItem() instanceof PolarBearArmor)
    ...
    private boolean hasAdvanceArmorSet(PlayerEntity player){
        ...
        for (ItemStack stack : armorList){
            if (!(stack.getItem() instanceof AdvancedPolarBearArmor)){
                return false;
            }
        }
        return true;
