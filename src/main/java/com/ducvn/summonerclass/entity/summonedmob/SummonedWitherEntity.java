...
public class SummonedWitherEntity extends WitherEntity implements ISummonedEntity {
    ...
    public SummonedWitherEntity(EntityType<? extends SummonedWitherEntity> p_i225714_1_, World p_i225714_2_) {
    ...
    public boolean isInvisibleTo(PlayerEntity player) {
        if (player.getScoreboardName().equals(this.getCustomName().getString().replace("'s wither",""))){
            return false;
        }
        else {
    ...
    public void setAttackTarget(LivingEntity entity, LivingEntity inputMaster){
        ...
        this.setAlternativeTarget(0, entity.getId());
    }
        ...
        if (!level.isClientSide){
            if (((ServerWorld) level).getEntity(target) == null
                ...
                if ((isSupreme && !hasNoArmor(level.getPlayerByUUID(master)))
                    ...
                    if (level.getPlayerByUUID(master).getHealth() > 1.0F){
                        level.getPlayerByUUID(master).setHealth(1.0F);
                    }
                }
            }
        }
